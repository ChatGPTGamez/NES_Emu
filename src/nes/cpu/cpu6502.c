...
static void service_interrupt(CPU6502* c, u16 vec_addr, bool set_break_flag)
{
    u16 ret = c->pc;
    push(c, (u8)((ret >> 8) & 0xFFu));
    push(c, (u8)(ret & 0xFFu));

    u8 p_push = (u8)(c->p | F_U);
    if (set_break_flag) p_push |= F_B;
    else p_push &= (u8)~F_B;
    push(c, p_push);

    c->p = (u8)(c->p | F_I);
    c->pc = vec_read16(c, vec_addr);
}
...
void op_ADC(CPU6502* c, u16 addr, bool has, bool pcross)
{
    (void)has;(void)pcross;
    u8 v = rd(c, addr);
    unsigned int carry = ((c->p & F_C) != 0u) ? 1u : 0u;
    unsigned int sum = (unsigned int)c->a + (unsigned int)v + carry;
    u8 res = (u8)sum;

    if (sum > 0xFFu) c->p |= F_C; else c->p &= (u8)~F_C;
...
{
    (void)a;(void)h;(void)p;
    // BRK is a 2-byte instruction; advance past padding byte.
    c->pc++;
    service_interrupt(c, 0xFFFE, true);
}
